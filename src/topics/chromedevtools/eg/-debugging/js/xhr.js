(function() {
	...
	var ajaxCall = function(success){
		var url = "../db/1loc.json";
		if(fetchCount > 2) {
			url = "../db/loc.json";
		}
		$.getJSON(url,function(d){
		  pageMem = (hasLocalStorage)? (ls.data = JSON.stringify(d)): JSON.stringify(d);//"{\"districts\":{\"a\":{}}}"
	...
	var locationLoadedSuccess = function(callback) {
		...
		if(!((hasLocalStorage && ls.data)) || !pageMem) {
		...
		if(typeof callback == "function") {
			...
			fetchCount = 0;
			if(hasLocalStorage && ls.data) {
				callback(JSON.parse(ls.data));
			}
			else if(pageMem) {
